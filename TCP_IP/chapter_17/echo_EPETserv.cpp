...
void setnonblockingmode(int fd){
    int flag = fcntl(fd, F_GETFL, 0);       //获取fd文件描述符的属性
    fcntl(fd, F_SETFL, flag | O_NONBLOCK);  //设置fd文件描述符的属性
}
...
int main(int argc, char *argv[]){
    ...
    setnonblockingmode(serv_sock);      //使用非阻塞使得I/O套接字
    ...
    while (1){
        ...
        //当监视的事件发生时，会将对应的时间写入到ep_events中，并返回发生监视事件的数量
        ...
        for (int i = 0 ; i < event_cnt; i++){
            ...
            if (ep_events[i].data.fd == serv_sock){     
                ...
                event.events = EPOLLIN | EPOLLET;;		//将客户端的读事件也加入到epoll中进行监听
                event.data.fd = clnt_sock;
                epoll_ctl(epfd, EPOLL_CTL_ADD, clnt_sock, &event);  //将clnt_sock上发生的读事件加入监听
                cout << "connect client : " << clnt_sock << endl;
            }else{
                while (1){      //边缘触发得方式中只会注册一次事件，需要读取缓冲区中的所有数据，所以需要循环调用read函数
